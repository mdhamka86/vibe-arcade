...
// Grand-finale "Lip Sync For Your Legacy" songs (bigger, more anthemic)
const LEGACY_SONGS = [
...
const TRIVIA = [
  ...
  { q: "Which spin-off crowned Lawrence Chaney as its winner?", o: ["Drag Race UK Series 2", "Canada's Drag Race", "Drag Race Down Under", "Drag Race España"], a: 0, ep: "Drag Race UK S2 Finale (2021)", line: "Scotland's sweetheart takes the crown." },
...
const LYRIC_LINES = [
  ...
  { line: "lip sync for your legacy", song: "The grand finale" },
...
// ---- Snatch Game: pick the funniest answer for a celebrity prompt ----
// Each round gives a celebrity persona + a fill-in prompt; you pick the funniest
// of 3 options. The "best" option is the wittiest in-character answer.
...
// Generate a lineup of queens with short, funny, unique descriptions for the Library.
...
function DragRaceGame() {
  ...
  // FINALE: Lip Sync For Your Legacy
  ...
  // Called when the player clears the board. Builds the full field (all queens
  // get a simulated time), ranks the player, finds the bottom two, asks RuPaul
  // for a punny reaction, then routes: bottom two lip sync, everyone else safe.
  const completeMemory = async (yourTime, yourMoves) => {
    if (memTimer.current) { clearInterval(memTimer.current); memTimer.current = null; }
    setMemDone(true);
    // simulate rival times. the star queen is fast; others vary around the player.
    const rivals = (cast.length ? cast : QUEENS.filter((q) => q !== dragName)).map((name) => {
      let base;
    ...
    // RuPaul's pun (top third = praise, bottom third = diss, middle = light praise)
    ...
    // stash routing info for the continue button
  ...
  const buildTea = async () => {
    setTeaLoading(true);
    // instant fallback from the static bank
    ...
    try {
      ...
      if (Array.isArray(ai) && ai.length >= 5 &&
        ...
        // only swap if the player hasn't started answering yet
        setTeaIndex((idx) => {
          setTeaCorrect((cor) => {
  ...
  const buildScramble = () => {
    // favor meatier phrases (4+ words) so it stays challenging; allow a short one occasionally
  ...
  const buildSnatch = () => {
    ...
    // player gets the first celeb; 3 rival queens get the next three
    const rivalPool = shuffle(cast.length ? cast : QUEENS.filter((q) => q !== dragName)).slice(0, 3);
    ...
    setChMax(1); // graded by rank, not count
  };

  // submit the player's snatch quote: AI writes rival quotes + ranks everyone
  const submitSnatch = async () => {
    ...
    try {
      ...
      // safety: make sure the player is present
  ...
  // from snatch results, route by the player's rank
  const finishSnatch = () => {
    const idx = snEntries.findIndex((e) => e.isPlayer);
    const total = snEntries.length || 1;
    // if the player somehow isn't in the list, treat as mid-pack (safe)
    const rank = idx === -1 ? Math.ceil(total / 2) : idx + 1;
    // top finish = win/high, middle = safe, bottom = lip sync
    let score;
    if (rank === 1) score = total;            // 100% -> WIN
    else if (rank <= Math.ceil(total / 3)) score = Math.ceil(total * 0.8); // HIGH
    else if (rank <= Math.ceil((total * 2) / 3)) score = Math.ceil(total * 0.6); // SAFE
    else score = 0;                            // BOTTOM
  ...
  const buildRunway = async () => {
    // start with a random static theme as instant fallback, then try AI for a fresh one
    ...
    try {
      ...
      // validate the AI response before trusting it
      if (ai && ai.theme && Array.isArray(ai.rounds) && ai.rounds.length >= 2 &&
        ...
        // only swap in the AI theme if the player hasn't started answering yet
        setMcIndex((idx) => {
          setMcCorrect((cor) => {
  ...
  // pick a random challenge type each week, never the same two weeks running
  ...
  // then show the "Meet Your Sisters" screen.
  const generateSeasonCast = async () => {
    ...
    try {
      ...
      // build name list, dedupe, ensure no clash with player or Nadine
      const personas = {};
      const names = [];
      ...
      // backfill from the static roster if the AI returned too few
      ...
      // Nadine is always part of the cast
  ...
  const flipCard = (uid) => {
    ...
    if (nowFlipped.length === 2) {
      ...
      // current matched pairs before this resolves
      ...
      setTimeout(() => {
        ...
        setMemLock(false);
        if (willWin) {
          // freeze the clock and finish; memTime is read via its setter to get the latest
  ...
  // bump placement up one tier on a great read (uses the banked/last score)
  ...
  // tally a weekly placement into run stats (for the end screen)
  ...
  // start a survival lip sync against a specific opponent (or random)
  const startSurvivalLipSync = (opponent) => {
    setLsSong(LIPSYNC_SONGS[Math.floor(Math.random() * LIPSYNC_SONGS.length)]);
    const pool = cast.length ? cast : QUEENS.filter((q) => q !== dragName);
  ...
  // Match Game: from the result screen, route the player. If they're bottom two,
  // they lip sync against the OTHER bottom-two queen. Otherwise they're safe.
  ...
  const advanceWeek = () => {
    const remaining = Math.max(2, queensLeft - 1);
    setQueensLeft(remaining);
    const pickFinalist = () => {
      const pool = cast.length ? cast : QUEENS.filter((q) => q !== dragName);
      // the strongest queen reaches the final more often than not
      if (pool.includes(STAR_QUEEN) && Math.random() < 0.45) return STAR_QUEEN;
      return pool[Math.floor(Math.random() * pool.length)];
    };
    if (remaining <= 2) {
      // FINAL TWO: Lip Sync For Your Legacy
      setFinalist(pickFinalist());
      ...
      // safety net: if weeks run out before final two, still go to finale
      setFinalist(pickFinalist());
      setLegSong(LEGACY_SONGS[Math.floor(Math.random() * LEGACY_SONGS.length)]);
      setLegStage("intro");
      setLegYou(0); setLegRival(0); setLegStamina(100); setLegRound(0);
  ...
  // ---- THE LIBRARY ----
  // Open the library: draw queens from this season's cast, using their personas.
  const openLibrary = async () => {
    setLibLoading(true); setAiError("");
    try {
      const rivalPool = (cast.length ? cast : QUEENS.filter((q) => q !== dragName));
      // the star queen always shows up; fill the rest from the cast
      const others = shuffle(rivalPool.filter((q) => q !== STAR_QUEEN)).slice(0, 3);
      const pool = shuffle([STAR_QUEEN, ...others]);
  ...
  // Submit the player's read of the currently selected target.
  const submitLibraryRead = async () => {
    ...
    try {
      ...
      // banking: the LAST graded read is the one that counts
  ...
  // Player chose to read again: clear the current attempt, back to reading.
  const readAgain = () => {
    setReadInput(""); setReadScore(null); setReadCritique(""); setReadTarget(null);
    setLibStage("reading");
  };

  // Player is done: generate the rival reads, build the leaderboard.
  const finishLibrary = async () => {
    ...
    try {
      ...
      // even if rival reads fail, let the player proceed with just their score
      setRivalReads([]);
      setLibStage("leaderboard");
    } finally { setLibLoading(false); }
  };
  ...
  // ---- lip sync "Top That" duel ----
  // 6 rounds. Each round has a crowd VIBE. You and the rival each commit a move
  // simultaneously. A move on-vibe scores high; off-vibe fizzles. The higher
  // round score wins that round and steals momentum. Win 2 rounds in a row to
  // arm a SHOWSTOPPER (next on-vibe win is worth double). Most points by the
  // final round takes the lip sync.
  const LS_TOTAL_ROUNDS = 6;

  // deterministic-ish vibe sequence per song so it feels composed, ending on a finish
  ...
  const scoreMove = (move, vibe) => {
    if (!move) return 0;
    if (move.fav === vibe.key) return move.base + 4;      // on-vibe: strong
    // off-vibe: noticeably weaker, but not a guaranteed loss
    return Math.max(1, move.base - move.risk - 1);
  };

  // Smart rival. She reads the vibe well (better each week) and plays the
  // strongest on-vibe move, so a sloppy pick from you will usually lose the
  // round. But she misreads often enough that one slip won't end the duel.
  const rivalPick = (vibe, rivalCombo) => {
    // the star queen reads almost flawlessly; everyone else scales with the week
  ...
  const playMove = (move) => {
    ...
    const tie = youPts === rivalPts;

    // showstoppers for BOTH queens: win 2+ rounds in a row on-vibe = double
    ...
    // recompute winner after any showstopper doubling (a doubled score can flip a tie)
  ...
  // ---- FINALE: Lip Sync For Your Legacy ----
  // A stamina-management duel over 5 passages. Each passage you choose:
  //  - Hold Back: small points, but RECOVER stamina (+18)
  //  - Go All Out: big points, costs stamina (-30); fails badly if you're low
  //  - Legacy Moment: ONE-TIME huge payout, only worth it on the final passage
  // The rival plays a strong, sensible pattern. Manage your meter, time your
  // moment, and outscore her by the final note to win the crown.
  const LEG_ROUNDS = 5;
  ...
  const legPlay = (choice) => {
    ...
    if (choice === "hold") {
      youGain = 6 + Math.floor(Math.random() * 3);     // 6-8
      stamCost = -18;                                   // recover
      note = "You pace yourself, saving energy for the big moments.";
    } else if (choice === "allout") {
      // effectiveness scales with available stamina
      ...
      // legacy moment: massive, but a let-down if you have no gas
      if (stam >= 25) { youGain = 26 + Math.floor(Math.random() * 6); note = "YOUR LEGACY MOMENT. A jaw-dropping reveal that brings the house DOWN."; }
    ...
    // rival: smart, steady. she goes all-out when she has gas, holds when low,
    // and unleashes her own big moment on the last passage. The star queen hits harder.
    let rivalGain;
  ...
  if (phase === "intro") {
    ...
    return (
      <div style={introWrap}>
        {/* ambient gold glow behind the marquee */}
        ...
        <div style={{ position: "relative", width: "100%", maxWidth: 480, textAlign: "center", zIndex: 1 }}>
          ...
          <div style={{ margin: "26px auto 8px", maxWidth: 420, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {ruPun("Make it to the final two and you'll sync for your legacy. The crown doesn't come to the timid. It comes to the fierce.")}
          </div>

          {/* the season's offerings, chic single panel */}
          <div style={{
          ...
          {/* entry button: gold, not the pink CTA */}
          <button
  ...
  if (phase === "challenge") {
    ...
    // ----- SNATCH GAME (write a quote in character) -----
    ...
    // ----- UNTUCK THE DRAMA & RUNWAY REALNESS (multiple choice) -----
  ...
  if (phase === "reading") {
    ...
    // STAGE: reading (pick a target + write)
    if (libStage === "reading") {
      return (
        <div style={wrap}>
          <Header />
    ...
    // STAGE: scored (your read result, then read again / done)
    if (libStage === "scored") {
      return (
        <div style={wrap}>
          <Header />
    ...
    // STAGE: leaderboard (rival reads + rankings, then close)
  ...
  if (phase === "legacy") {
      ...
      return (
        <div style={wrap}>
          ...
          <div style={{ ...card, borderColor: "#ffd966" }}>
            ...
            <div style={{ ...title, fontSize: 30, marginTop: 4 }}>LIP SYNC FOR YOUR LEGACY</div>
            <p style={{ fontSize: 15, lineHeight: 1.6, marginTop: 10 }}>
              It's down to you and <b style={{ color: "#d8b15a" }}>{finalist}</b>. One final lip sync decides the crown. This isn't survival, it's <b>legacy</b>.
            </p>
            ...
            <div style={{ textAlign: "left", fontSize: 13.5, lineHeight: 1.7, margin: "14px 0", opacity: 0.92 }}>
              ...
              <b>Legacy Moment</b>: one-time showstopper, save it for the finish<br />
    ...
    // STAGE done: final result, route to winner or runner-up
    if (legStage === "done") {
    ...
    return (
      <div style={wrap}>
        ...
        <div style={{ ...card, maxWidth: 480, borderColor: "#ffd966" }}>
          <div style={{ fontSize: 13, opacity: 0.7, letterSpacing: 2 }}>LIP SYNC FOR YOUR LEGACY</div>
          ...
          {!rr ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <button style={{ ...btn, background: legMomentUsed ? "rgba(120,120,140,0.3)" : "linear-gradient(90deg,#ffd966,#ff9d3d)", color: legMomentUsed ? "#888" : "#3a1240", pointerEvents: legMomentUsed ? "none" : "auto" }} onClick={() => legPlay("moment")}>
                Legacy Moment {legMomentUsed ? "(used)" : "(one time only)"}
              </button>
            </div>
          ) : (
            <div>
      ...
      <div style={wrap}>
        ...
        <div style={card}>
          ...
          <p style={{ fontSize: 15, lineHeight: 1.6 }}>
            You made it all the way to the final two and lip synced for your legacy, but <b style={{ color: "#d8b15a" }}>{finalist}</b> took the crown this time. A legend in your own right, <b style={{ color: "#ffd966" }}>{dragName}</b>. So close to the throne you could taste it.
          </p>
    ...
    return (
      <div style={wrap}>
        ...
        <div style={{ ...card, borderColor: "#ffd966" }}>
          ...
          <p style={{ fontSize: 16, lineHeight: 1.6 }}>You out-performed <b style={{ color: "#d8b15a" }}>{finalist}</b> in the Lip Sync For Your Legacy and snatched the crown. <b style={{ color: "#ffd966" }}>{dragName}</b>, you are the next drag superstar.</p>
