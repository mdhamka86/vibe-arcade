...
function findPdfs(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    if (t.indexOf("integrated") !== -1) pdfs.card = abs(m[1]);
    else if (t.indexOf("selection") !== -1 || t.indexOf("media") !== -1) pdfs.tips = abs(m[1]);
    else if (t.indexOf("analysis") !== -1) pdfs.analysis = abs(m[1]);
  }
  return pdfs;
}
