...
async function actIdeas(force) {
  ...
  const s = await loadAll();
  const [news, rates] = await Promise.all([getNews('forex'), refRates()]);
...
{"ideas":[{"pair":"EUR/USD","direction":"BUY|SELL","entry_zone":"1.1440-1.1455","tp":"1.1520","sl":"1.1400","lots":"0.05","horizon":"intraday-48h","conviction":"LOW|MED|HIGH","thesis":"...","risks":"...","margin_note":"...","correlation_note":"..."}],"stand_down":false,"desk_note":"one-paragraph read of the session"}`;
  ...
  const ideas = await claude(prompt, 2200);
  ideas.generatedAt = t.iso;
  ideas.dateKey = t.dateKey;
  await rSet(key, ideas);

  // log offered ideas into ledger
  for (const idea of ideas.ideas || []) {
    s.ledger.push({ id: `idea_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, ts: Date.now(), date: t.dmy, status: 'offered', idea });
  }
...
Fresh wire:\n${digest(news, 14)}
Write a tight morning briefing. JSON only: {"headline":"one line","lines":[{"pair":"...","read":"1-2 sentences on how overnight news touches this position"}],"calendar_watch":"today's likely landmine events and rough Bangkok timing","overall":"2-3 sentences"}`, 1400);
  }
