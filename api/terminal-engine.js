...
// Stronger identity check for recognising a HELD position across syncs. samePos is the
// base (pair+direction+lots+entry). When BOTH sides carry a real MT5 open time, an exact
// open-time match is the most reliable signal there is — it survives tiny price re-reads.
function sameHeldPosition(known, screen) {
  const kT = known.mt5OpenTs ?? parseMT5Time(known.openTime);
  const sT = parseMT5Time(screen.openTime);
  // if both have a real open time and same pair+direction, trust the open time as identity
  if (kT != null && sT != null &&
      normPair(known.pair) === normPair(screen.pair) &&
      (known.direction || '').toUpperCase() === (screen.direction || '').toUpperCase()) {
    return Math.abs(kT - sT) < 60000; // within a minute = same open = same trade
  }
  // otherwise fall back to the standard price-based match
  return samePos(known, screen);
}
...
async function actSync(positionsImgs, historyImg) {
  ...
  for (const p of s.book.positions) {
    const onScreen = seen.find((x) => samePos(p, x));
    if (onScreen) {
  ...
  report.savedPositions = still.length; // diagnostic: how many positions the engine persisted
