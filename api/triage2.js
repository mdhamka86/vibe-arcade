...
function scanCard(html) {
  ...
  // Work in a bounded window around "RACE 1:" so the header regexes never scan
  // the whole 800KB page (that caused the function timeout).
  let r1 = "(R1 not found)";
  const r1pos = plain.search(/RACE\s*1:/i);
  if (r1pos !== -1) {
    const window = plain.slice(r1pos, r1pos + 400);
    const hm = window.match(
      /RACE\s*1:.*?PRIZE MONEY\s*:?\s*[^|]*?(?=\s+NO\.\s+HORSE|\s+Horse No|RACE\s*2:|$)/i
    );
    r1 = hm ? hm[0].trim() : (window.match(/RACE\s*1:.{0,140}/i) || ["(R1 not found)"])[0];
    r1 = r1.replace(/\s+NO\.\s+HORSE NAME.*$/i, "").trim();
  }
  ...
  // PDFs: find every ".pdf" via indexOf (fast, no regex backtracking on huge
  // pages — the greedy-regex version timed the function out), walk back to the
  // URL start, classify by filename (SP naming: _FORM_, _TIPSHEET_, _RFA_).
  const pdfs = {};
