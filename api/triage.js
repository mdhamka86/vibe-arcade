...
function scanCard(html) {
  ...
  // PDFs appear as MARKDOWN links [label](url.pdf) in the rendered text, and
  // sometimes as <a href> in raw HTML. Catch both.
  const pdfs = {};
  ...
  while ((p = mdRe.exec(plain)) !== null) classify(p[1], p[2]);
  const aRe = /<a[^>]+href="([^"]+\.pdf)"[^>]*>([^<]*)<\/a>/gi;
  while ((p = aRe.exec(html)) !== null) classify(p[2], p[1]);
