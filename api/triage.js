...
function scanCard(html) {
  ...
  const races = (text.match(/RACE\s*\d+:/gi) || []).length;
  ...
  while ((row = rowRe.exec(html)) !== null) {
    ...
    if (c && /^\d{1,2}$/.test(c[1].replace(/<[^>]+>/g, "").trim())) runners++;
  }
  const avgField = races ? Math.round(runners / races) : 0;
  ...
  while ((p = pdfRe.exec(html)) !== null) {
    ...
    if (t.indexOf("integrated") !== -1) pdfs.card = abs(p[1]);
    else if (t.indexOf("selection") !== -1 || t.indexOf("media") !== -1) pdfs.tips = abs(p[1]);
    else if (t.indexOf("analysis") !== -1) pdfs.analysis = abs(p[1]);
  }
  const docCount = Object.keys(pdfs).length;

  return { r1, dist, prize, races, avgField, hasGraded, hasHandicapSpread, maidenHeavy, pdfs, docCount };
}

function scoreMeet(s) {
  const docScore = Math.min(s.docCount, 3) / 3 * 40;
  const fieldScore = Math.min(s.avgField || 0, 14) / 14 * 20;
  const raceScore = Math.min(s.races || 0, 12) / 12 * 15;
  let classScore = s.hasGraded ? 25 : s.hasHandicapSpread ? 15 : 5;
  if (s.maidenHeavy) classScore = Math.max(5, classScore - 10);
  const total = Math.round(docScore + fieldScore + raceScore + classScore);
  const bits = [];
  bits.push(s.docCount ? s.docCount + " analysis doc" + (s.docCount > 1 ? "s" : "") : "no analysis docs");
  bits.push(s.races + " races");
  bits.push("~" + s.avgField + " avg field");
      ...
      todays.map(async (mt) => {
        try {
          ...
          return {
            ...
            races: s.races, avgField: s.avgField, pdfs: s.pdfs, docCount: s.docCount,
          };
        } catch (e) {
